...
const JobPostSearch = () => {
  ...
  const handleSaveKeyworLocalStorage = (kw) => {
    try {
      if (kw) {
        const keywordListStr = localStorage.getItem('imyanya_search_history'); // updated key
        ...
        if (
          ...
          if (!keywordList.includes(kw)) {
            if (keywordList.length >= 5) {
              localStorage.setItem(
                'imyanya_search_history', // updated key
                JSON.stringify([
                  kw,
          ...
          localStorage.setItem('imyanya_search_history', JSON.stringify([kw])); // updated key
        }
      }
    } catch (error) {
